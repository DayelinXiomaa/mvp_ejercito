// Kiosk Offline & Fast-Loading Service Worker - Armas y Servicios
const CACHE_NAME = 'kiosk-armas-cache-v3';
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      return cache.addAll(PRECACHE_URLS).catch((err) => {
        console.warn('[SW] Precache inicial parcial:', err);
      });
    })
  );
});
...
// Soporte para peticiones parciales HTTP 206 (necesario para video y audio HTML5)
...
self.addEventListener('fetch', (event) => {
  ...
  // Solo peticiones GET HTTP/HTTPS
  ...
  // Detección de archivos multimedia o estáticos pesados (Imágenes, audios, fuentes, css, js)
  const isStaticAsset = (
    ...
    event.respondWith(
      caches.open(CACHE_NAME).then(async (cache) => {
        ...
        try {
          ...
          // Solo cachear respuestas 200 completas
  ...
  // Para navegación SPA (HTML): Network first con fallback a cache local
  ...
  // Resto de recursos (ej. JSONs de contenido): Stale-While-Revalidate
  event.respondWith(
    caches.open(CACHE_NAME).then(async (cache) => {
