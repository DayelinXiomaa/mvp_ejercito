...
export const TranslationsVisualEditor: React.FC<TranslationsVisualEditorProps> = ({
  ...
  const getCategory = (k: string): string => {
    ...
    if (k.startsWith('div_') || k.startsWith('brig_') || k.includes('division') || k.includes('region')) return 'division';
    if (k.startsWith('btn_') || k === 'close' || k.includes('back')) return 'buttons';
    if (k.startsWith('filter_') || k.includes('search')) return 'filters';
    if (k.startsWith('tab_') || k.includes('organigrama') || k.includes('resena')) return 'tabs';
    return 'other';
  };
  ...
  const categories = [
    ...
    { id: 'division', label: 'Divisiones / Brigadas', count: allKeys.filter((k) => getCategory(k) === 'division').length },
    ...
    { id: 'tabs', label: 'Pestañas', count: allKeys.filter((k) => getCategory(k) === 'tabs').length },
