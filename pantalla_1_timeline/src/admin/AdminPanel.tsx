...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout, onExitAdmin, defaultScreenId }) => {
  ...
  const [langs, setLangs] = useState<unknown>(null);
  ...
  const loadContent = () => {
    ...
    api
      ...
      .then(setContent)
      .catch((e) => setMsg('Error al leer contenido: ' + e.message))
  ...
  const loadLanguages = () => {
    api
      .getLanguages(screen)
      .then(setLangs)
      .catch((e) => setMsg('Error al leer traducciones: ' + e.message));
  };
  ...
  const loadMedia = () => {
    api
      .listMedia(screen)
      .then(setMedia)
      .catch((e) => setMsg('Error al listar media: ' + e.message));
  };
  ...
  const saveContent = async () => {
    ...
    try {
      ...
      setMsg('Contenido guardado correctamente ✔');
    } catch (e) {
  ...
  const saveLanguages = async () => {
    ...
    try {
      ...
      setMsg('Traducciones guardadas correctamente ✔');
    } catch (e) {
      setMsg('Error: ' + (e as Error).message);
    } finally {
      setBusy(false);
